...
export const FileUploader = forwardRef<HTMLDivElement, FileUploaderProps & React.HTMLAttributes<HTMLDivElement>>(
  (
    { className, dropzoneOptions, value, onValueChange, reSelect = false, orientation = 'vertical', children, dir, ...props },
    ref
  ) => {
    ...
    return (
      <div className={cn('w-full px-1')} ref={containerRef} aria-description='content file holder'>
        <div
          {...props}
          ref={ref}
          className={cn('flex gap-1 rounded-xl', orientation === 'horizontal' ? 'flex-row flex-wrap' : 'flex-col', className)}
        >
          {children}
        </div>
      </div>
  ...
  ({ className, index, children, ...props }, ref) => {
    ...
    return (
      <div
        ...
        {...props}
      >
        <div className='flex h-full w-full items-center gap-1.5 leading-none font-medium tracking-tight'>{children}</div>
        <button
          type='button'
